...
//       .append('<div class="message in "><div class="sender"><a href="javascript:void(0);" title="Rufat Askerov"><img src="img/avatar.png" class="avatar" alt="Rufat Askerov"></a></div><div class="body"><div class="content"><span>Hello, welcome to chat </span></div><div class="seen"><span>few seconds ago </span> </div></div><div class="clear"></div></div>');
...
$(document).on("click", "#mmc-chat .preview-image", function () {
  preview =
    '<div class="preview-overlay"><div class="preview-placeholder"><img class="preview-image" src="{0}"/><div class="preview-caption">{1}</div></div></div>';
  ...
  var _left =
    $(document).scrollLeft() +
    windowWidth / 2 -
    origin.parent(".preview-placeholder").offset().left -
    newWidth / 2;
  var _top =
    $(document).scrollTop() +
    windowHeight / 2 -
    origin.parent(".preview-placeholder").offset().top -
    newHeight / 2;
...
$(document).on(
  "click",
  '#mmc-chat .preview-overlay:not(".preview-placeholder")',
  function () {
...
$(document).on(
  "click",
  "#mmc-chat .box .stickers .tab-content li",
  function () {
    _sendMsg = $(this).parents(".box-footer").find(".send-message div");
    //_src = $(this).find('img').attr('src');
    _img = $(this).html();
    if (
      $(this).parents(".box-footer").find(".send-message div").html() ==
      '<span class="placeholder">{0}</span>'.format(
        _sendMsg.data("placeholder")
      )
    ) {
      _sendMsg.html(_img);
    } else {
...
$(document).on("keypress", "#mmc-chat .textarea", function (e) {
  if (e.which == 13) {
    ...
    _msgTxt = $(this).html();
    _box_message.append(
      '<div class="message out no-avatar"> <div class="sender"> <a href="javascript:void(0);" title="Rufat Askerov"> <img src="img/avatar.png" class="avatar" alt="Rufat Askerov"> </a> </div><div class="body"> <div class="content">{0}</div><div class="seen"><span>a few seconds ago </span> </div></div><div class="clear"></div></div>'.format(
        _msgTxt
      )
    );
